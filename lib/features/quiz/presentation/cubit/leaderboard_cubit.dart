...
class LeaderboardCubit extends Cubit<LeaderboardState> {
  ...
  Future<void> load() async {
    ...
    try {
      ...
      print(
        '[LeaderboardCubit] calling _repository.loadData() ��� isClosed=$isClosed',
      );
      ...
      if (isClosed) {
        print(
          '[LeaderboardCubit] cubit closed after loadData() — aborting (this confirms navigation-away as root cause)',
        );
        return;
      }
      ...
      print(
        '[LeaderboardCubit] after getTopUsers() — isClosed=$isClosed, entries=${entries.length}',
      );
      if (isClosed) {
      ...
      bool? userShowInLeaderboard;
      DateTime? userLastVisibilityToggleAt;

      if (user != null && !user.isAnonymous) {
        // Fetch rank and entry in parallel
        print(
          '[LeaderboardCubit] calling Future.wait for rank+entry — isClosed=$isClosed',
        );
        final results = await Future.wait([
          _repository.getUserRank(user.uid),
          _repository.getUserEntry(user.uid),
          _repository.getLeaderboardVisibilityPreference(uid: user.uid),
        ]);
        // DEBUG: check if cubit was closed while awaiting Future.wait
        print('[LeaderboardCubit] after Future.wait — isClosed=$isClosed');
        if (isClosed) {
          print(
            '[LeaderboardCubit] cubit closed after Future.wait — aborting (confirms back-navigation during Firestore fetch)',
          );
          return;
        }
        userRank = results[0] as int?;
        userEntry = results[1] as LeaderboardEntry?;
        final visibility = results[2] as LeaderboardVisibilityPreference;
        userShowInLeaderboard = !visibility.isAnonymous;
        userLastVisibilityToggleAt = visibility.lastToggleAt;
        ...
        if (userEntry == null && _repository.totalAnswered > 0) {
          userEntry = LeaderboardEntry(
            ...
            displayName:
                sanitizeUtf16(
                  user.displayName,
                  fallback: di.sl<SettingsService>().getAppLanguage() == 'ar'
                      ? 'مستخدم'
                      : 'User',
                ),
        ...
        if (_repository.totalAnswered > 0) {
          userEntry = LeaderboardEntry(
            uid: 'local',
            displayName: di.sl<SettingsService>().getAppLanguage() == 'ar'
                ? 'أنت'
                : 'You',
            totalScore: _repository.totalScore,
            streak: _repository.streak,
      ...
      print(
        '[LeaderboardCubit] about to emit LeaderboardLoaded — isClosed=$isClosed',
      );
      if (isClosed) {
      ...
      emit(
        LeaderboardLoaded(
          ...
          currentUserShowInLeaderboard: userShowInLeaderboard,
          currentUserLastVisibilityToggleAt: userLastVisibilityToggleAt,
        ),
      );
    } catch (e) {
      ...
      if (isClosed) {
        print(
          '[LeaderboardCubit] cubit already closed — suppressing error emit',
        );
        return;
      }
  ...
  Future<void> updateVisibilityPreference(bool showInLeaderboard) async {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null || user.isAnonymous) return;
    await _repository.updateLeaderboardVisibilityPreference(
      uid: user.uid,
